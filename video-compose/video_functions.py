...
def input_has_audio(path: Path) -> bool:
    """Return True if the input file has at least one audio stream (via ffprobe)."""
    try:
        res = subprocess.run(
            [
                "ffprobe", "-v", "error",
...
def action_add_sound(input_video: Path, output_video: Path, sound: Path, start: float, duration: Optional[float] = None, volume: Optional[float] = None) -> None:
    ...
    # Determine whether to mix with existing audio or output only the FX as audio
    has_aud = input_has_audio(input_video)
    if has_aud:
        filter_complex = (
        ...
        # No audio in input: just use the processed FX as the sole audio track
        filter_complex = f"[1:a]{fx_filter}[fx]"
        cmd = [
            "ffmpeg", "-hide_banner", "-y",
            "-i", input_video.as_posix(),
            "-i", sound.as_posix(),
...
def action_animate_text_with_lottie(
    ...
    overlay_w: int = 512,
    overlay_h: int = 512,
    overlay_js_path: Optional[Path] = None,
) -> None:
    """Overlay a Lottie animation over the video by invoking the node-based renderer/overlay script.
    Requires overlay.js (see README) and Node.js.
    """
    ensure_node()
    ensure_ffmpeg()  # overlay.js ends with ffmpeg overlay

    # Resolve overlay.js path: use provided path or default to overlay.js next to this script
    overlay_js = overlay_js_path if overlay_js_path else (Path(__file__).parent / "overlay.js")
...
def action_running_code_drawtext(
    ...
    # Escape text for ffmpeg drawtext: backslashes, colons, and single quotes
    escaped_text = (
        text.replace('\\', r'\\')
            .replace(':', r'\:')
            .replace("'", r"\'")
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    p.add_argument("--overlay_js", help="Path to overlay.js to render Lottie and overlay (optional)")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if args.action in ("animate_text", "animate_text_with_lottie"):
        ...
        action_animate_text_with_lottie(
            ...
            overlay_js_path=Path(args.overlay_js).resolve() if args.overlay_js else None,
        )
        return 0
